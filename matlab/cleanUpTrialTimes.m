...
if (fid ~= -1)
    ...
    for lineIdx = 1:length(C{1})  % For each line
        mdIdx = mdIdx + 1;
        % Check to see if the last trial got recorded in the matlab video file.  Sometimes, rarely, it does not, so then prepopulate
        if (mdIdx > length(allTrialMarkers))
            break;
            stimLocX = getStimLocFromActions(C, lineIdx);
            actLocX = getActionLocFromActions(C, lineIdx);
            if (stimLocX == actLocX)
                allTrialMarkers(mdIdx) = allTrialMarkers(mdIdx-1);
                allTrialMarkers(mdIdx).AbsTime = allTrialMarkers(mdIdx).AbsTime + 2;
                allTrialMarkers(mdIdx).FrameNumber = allTrialMarkers(mdIdx).FrameNumber + 2*fps;
                allTrialMarkers(mdIdx+1) = allTrialMarkers(mdIdx);
            else
        ...
        % UNCOMMENT if want verbose output
...
if (length(newTrialEnds) <= length(C{1}))
    ...
    if (length(newTrialEnds) < length(C{1}))
        disp('Truncated 1 or more trials at the end, due to incomplete video metadata .mat file');
    end
else
