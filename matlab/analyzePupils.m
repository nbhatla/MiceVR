...
% 2023-01-12 - enable tracking of many days
...
colorLeftOnly = [80 23 94]/255; % purple
colorRight = [209 121 21]/255; % orange - old color: [226 50 50]/255; % red
colorRightOnly = [214 27 24]/255; % bright red
colorCenter = [15 157 88]/255;  % green

colorLeftFar = [2 193 193]/255; % cyan 
colorRightFar = [242 183 124]/255; % orange

shadingColorLeft = [214 227 252]/255;  % dull blue -         214, 227, 252
shadingColorLeftOnly = [212 192 252]/255; % dull purple -    212, 192, 252
shadingColorLeftFar = [0.84 0.98 0.99]; % dull cyan
shadingColorRight = [1 0.87 0.71]; % dull orange -          255, 222, 184
shadingColorRightOnly = [1 0.73 0.71]; % dull red -         255, 185, 184
...
stimEyeMoveTrialsExt = cell(2, 2); % 2 extinction on 3-choice (don't support 4-choice yet) x 2 eyes
actionEyeMoveTrials = {};
stimActionEyeMoveTrials = {};  % first axis is stimIdx, second is actionIdx, third is left/right eye
stimActionEyeMoveTrialsExt = {};
stimOptoEyeMoveTrials = {}; % first axis is stimLoc, second is optoState (OFF, LEFT, RIGHT, or BOTH), third is left/right eye
stimOptoEyeMoveTrialsExt = cell(2, 4, 2);

% These arrays store the number of trials for each eye (2) kept and total for LC, LO, RC and RO trial types
keptCnt = zeros(4,2);
allCnt = zeros(4,2);
...
for dIdx=1:numDays
    ...
    disp(['====================='])
    disp(['ANALYZING DAY ' num2str(days(dIdx))]);
    disp(['====================='])
    analFrameLim = [1 0];

    if (days(dIdx) < 10)
        dayStr = ['00' num2str(days(dIdx))];
    elseif (days(dIdx) < 100)
        dayStr = ['0' num2str(days(dIdx))];
    else
        dayStr = num2str(days(dIdx));
    end
    ...
    % These legends are used for the elevation and azimuth plots, and no other plots
    ...
    if (useEyeMeasurements && isKey(mouseToRpLine, mouseName))
        ...
        slope = [-0.142 -0.142];  % This is Stahl 2002 - I believe we used it, instead of what we measured with our own mice, because it allowed us to still be conservative and overestimate eye movements
        yint = [0.925 0.925];  % old defaults from paper: [1.055 1.055]
        disp('Eye measurements NOT FOUND, so using default values');
    end
    clear Rp  % wipe away becuase for each session, the video duration is different so the Rp dimensions will be different
    ...
    if (actionsFile ~= -1) % File found
        ...
        for i=1:length(trialRecs{1})
            ...
            % 1 if LO or RO, 0 otherwise
            extIdx = getExtinction(trialRecs, i);
            extIdxs = [extIdxs extIdx];
        ...
        elseif (currWorldNumStim == 2 || currWorldNumStim == 3)
            if (length(levelName{currWorldIdx}) == 1)  % It is the normal 2-choice or 3-choice level
                if (stimIdxs(i) == 0)
                    stimPatchColors(1,i,:) = shadingColorLeft;
                    if (extIdxs(i) == 1)
                        stimPatchColors(1,i,:) = shadingColorLeftOnly;
                    end
                elseif (stimIdxs(i) == 1)
                    stimPatchColors(1,i,:) = shadingColorRight;
                    if (extIdxs(i) == 1)
                        stimPatchColors(1,i,:) = shadingColorRightOnly;
                    end
                elseif (stimIdxs(i) == 2)
    ...
    % Collect data for averaging...  For multi-day analysis, pool these variables across days before averaging!
    totalNumStim = sum(numStim);
    trimmedStimIdxs = stimIdxs(1+numTruncatedStart:end-numTruncatedEnd);
    trimmedExtIdxs = extIdxs(1+numTruncatedStart:end-numTruncatedEnd);
    ...
    if (dIdx == 1)  % setup variables that will be added to on subsequent days
        ...
        stimActionEyeMoveTrialsExt = cell(2, totalNumStim, 2);
    ...
    for i=1:length(trimmedStimIdxs)
        extIdx = trimmedExtIdxs(i);
        if (trimmedWorldIdxs(i) == 0)
            stimIdx = trimmedStimIdxs(i);
            actionIdx = trimmedActionIdxs(i);
        elseif (trimmedWorldIdxs(i) == 1)
            stimIdx = numStim(1) + trimmedStimIdxs(i);
            actionIdx = numStim(1) + trimmedActionIdxs(i);
        else
            error('only supports levels with a maximum of 2 worlds - edit code to support more.');
        end

        if (stimIdx == -1) % catch trial
            for j=1:2
        ...
        % Added 10/25/20, removed ext 1/9/23 - Filter out correction trials from average graphs
        if (getCorrection(trialRecs, i))
            continue;
        end
        ...
        for j=1:2
            if (extIdx == 0)  % not an extinction trial, so pool accordingly
                ...
                sa = stimActionEyeMoveTrials{stimIdx+1, actionIdx+1, j};
                sa{end+1} = azimDeg(trimmedStarts(i):trimmedEnds(i)-1, j);
                stimActionEyeMoveTrials{stimIdx+1, actionIdx+1, j} = sa;

                osIdx = optoStates(i) + 2;  % Shift from the read value {-1,2) to the index (1,4)
                so = stimOptoEyeMoveTrials{stimIdx+1, osIdx, j};
                so{end+1} = azimDeg(trimmedStarts(i):trimmedEnds(i)-1, j);
                stimOptoEyeMoveTrials{stimIdx+1, osIdx, j} = so;
            else  % is an extinction trial, so pool accordingly
                s = stimEyeMoveTrialsExt{stimIdx+1, j};
                s{end+1} = azimDeg(trimmedStarts(i):trimmedEnds(i)-1, j);
                stimEyeMoveTrialsExt{stimIdx+1, j} = s;

                sa = stimActionEyeMoveTrialsExt{stimIdx+1, actionIdx+1, j};
                sa{end+1} = azimDeg(trimmedStarts(i):trimmedEnds(i)-1, j);
                stimActionEyeMoveTrialsExt{stimIdx+1, actionIdx+1, j} = sa;

                osIdx = optoStates(i) + 2;  % Shift from the read value {-1,2) to the index (1,4)
                so = stimOptoEyeMoveTrialsExt{stimIdx+1, osIdx, j};
                so{end+1} = azimDeg(trimmedStarts(i):trimmedEnds(i)-1, j);
                stimOptoEyeMoveTrialsExt{stimIdx+1, osIdx, j} = so;
            end

            a = actionEyeMoveTrials{actionIdx+1, j};
            a{end+1} = azimDeg(trimmedStarts(i):trimmedEnds(i)-1, j);
            actionEyeMoveTrials{actionIdx+1, j} = a;
    ...
    for i=1:size(azimDeg,2) % for each eye, fill in the NaNs
        ...
        % Stop plotting histograms for now (Jan 2023)
        %{
        figure;
    ...
    keptCnt(3, 1) = keptCnt(3, 1) + LeyeKeptRCCnt;
    keptCnt(4, 1) = keptCnt(4, 1) + LeyeKeptROCnt;

    allCnt(1, 1) = allCnt(1, 1) + LeyeAllLCCnt;
    allCnt(2, 1) = allCnt(2, 1) + LeyeAllLOCnt;
    allCnt(3, 1) = allCnt(3, 1) + LeyeAllRCCnt;
    allCnt(4, 1) = allCnt(4, 1) + LeyeAllROCnt;
    
    keptCnt(1, 2) = keptCnt(1, 2) + ReyeKeptLCCnt;
    keptCnt(2, 2) = keptCnt(2, 2) + ReyeKeptLOCnt;
    keptCnt(3, 2) = keptCnt(3, 2) + ReyeKeptRCCnt;
    keptCnt(4, 2) = keptCnt(4, 2) + ReyeKeptROCnt;

    allCnt(1, 2) = allCnt(1, 2) + ReyeAllLCCnt;
    allCnt(2, 2) = allCnt(2, 2) + ReyeAllLOCnt;
    allCnt(3, 2) = allCnt(3, 2) + ReyeAllRCCnt;
    allCnt(4, 2) = allCnt(4, 2) + ReyeAllROCnt;    

end  % Iterate through all the data files before plotting the averages

% OUTPUT TALLIED RESULTS AS TEXT
...
for eye = 1:2 
    ...
    disp([whichEye ' EYE ANALYZED:']);
    disp(['LC trials kept = ' num2str(round(100 * keptCnt(1,eye) / allCnt(1,eye), 0)) '% (' num2str(keptCnt(1,eye)) '/' num2str(allCnt(1,eye)) ')']);
    disp(['LO trials kept = ' num2str(round(100 * keptCnt(2,eye) / allCnt(2,eye), 0)) '% (' num2str(keptCnt(2,eye)) '/' num2str(allCnt(2,eye)) ')']);
    disp(['RC trials kept = ' num2str(round(100 * keptCnt(3,eye) / allCnt(3,eye), 0)) '% (' num2str(keptCnt(3,eye)) '/' num2str(allCnt(3,eye)) ')']);
    disp(['RO trials kept = ' num2str(round(100 * keptCnt(4,eye) / allCnt(4,eye), 0)) '% (' num2str(keptCnt(4,eye)) '/' num2str(allCnt(4,eye)) ')']);
end
...
resizedStimEyeExt = cell(size(stimEyeMoveTrialsExt));
mExt = cell(size(stimEyeMoveTrialsExt));
semExt = cell(size(stimEyeMoveTrialsExt));
ySemExt = cell(size(stimEyeMoveTrialsExt));
for eye=1:2  % For each eye
    h = [];  % handle for the figure used later
    n = 0;
    extraStim = 0;  % used for adding legend entries if extinction trials exist
    trialCnt = [];
    stimEyeLengths = [];
    for stimIdx=1:totalNumStim
        if (~isempty(stimEyeMoveTrials{stimIdx, eye})) % Prevents error if there are no stim of a type in a session
            ...
            trialCnt(stimIdx) = length(stimEyeMoveTrials{stimIdx,eye});
        end
    end
    % Next, add the extinction trials as well
    for extIdx=1:2
        if (~isempty(stimEyeMoveTrialsExt{extIdx, eye})) % Prevents error if there are no stim of a type in a session
            stimEyeLengths = [stimEyeLengths cellfun(@(x) length(x), stimEyeMoveTrialsExt{extIdx,eye})];
            n = n + length(stimEyeMoveTrialsExt{extIdx,eye});
            extraStim = extraStim + 1;
            trialCnt(totalNumStim + extIdx) = length(stimEyeMoveTrialsExt{extIdx,eye});
        end
    end
    ...
    % Next, renormalize the extinction trials' eye movements as well
    for extIdx=1:2
        if (~isempty(stimEyeMoveTrialsExt{extIdx,eye}))
            resizedStimEyeExt{extIdx,eye} = cellfun(@(x) interp1(1:length(x), x, 1:length(x)/minLengths(eye):length(x))',...
                                                    stimEyeMoveTrialsExt{extIdx,eye}, 'UniformOutput', false);
            d = cell2mat(resizedStimEyeExt{extIdx,eye}(:)');
            mExt{extIdx,eye} = nanmean(d, 2);
            semExt{extIdx,eye} = nanstd(d, [], 2) ./ sqrt(size(d, 1));
        end
    end
    ...
    % Do it for extinction trials too
    for extIdx=1:2
        ySemExt{extIdx,eye} = [mExt{extIdx,eye}'-semExt{extIdx,eye}', fliplr(mExt{extIdx,eye}'+semExt{extIdx,eye}')];
    end
    ...
    for extIdx=1:2        
        if (numStim(worldIdx) == 2 || numStim(worldIdx) == 3)
            if (length(levelName{worldIdx}) == 1)  % Regular 2-choice or 3-choice level
                if (extIdx == 1)
                    curColor = colorLeftOnly;
                    curShadingColor = shadingColorLeftOnly;
                    sLoc = 'Left Only';
                elseif (extIdx == 2)
                    curColor = colorRightOnly;
                    curShadingColor = shadingColorRightOnly;
                    sLoc = 'Right Only';
                end
            end
        end
        
        if (~isempty(ySemExt{extIdx, eye}))
            patch(ySemExt{extIdx,eye}, xSem, curShadingColor, 'EdgeColor', 'none');
            alpha(varianceAlpha);
            h = [h plot(mExt{extIdx,eye}, x, 'Color', curColor, 'LineWidth', lw)];
        end
    end
    ...
    title([mouseName ' ' num2str(days) ': ' eyeName ' eye, Correct & Incorrect Trials'], 'Interpreter', 'none');
...
resizedStimEyeExt = cell(size(stimEyeMoveTrialsExt));
muExt = cell(size(stimEyeMoveTrialsExt));
semExt = cell(size(stimEyeMoveTrialsExt));
ySemExt = cell(size(stimEyeMoveTrialsExt));
...
for eye=1:2  % For each eye
    ...
    trialCnt = [];
    % First collect all the trial lengths for the correct non-extinction trials, to assist with normalizing trial duration
    for stimIdx=1:totalNumStim
        ...
        if (~isempty(stimActionEyeMoveTrials{stimIdx,stimIdx,eye}))
            ...
            trialCnt(stimIdx) = length(stimActionEyeMoveTrials{stimIdx,stimIdx,eye});
        end
    end

    % Next, add the extinction trials as well
    for extIdx=1:2
        if (~isempty(stimActionEyeMoveTrialsExt{extIdx,extIdx,eye}))
            stimCorrectEyeLengths = [stimCorrectEyeLengths cellfun(@(x) length(x), stimActionEyeMoveTrialsExt{extIdx,extIdx,eye})];
            n = n + length(stimActionEyeMoveTrialsExt{extIdx,extIdx,eye});
            extraStim = extraStim + 1;
            trialCnt(totalNumStim + extIdx) = length(stimActionEyeMoveTrialsExt{extIdx,extIdx,eye});
        end
    end
    ...
    for extIdx=1:2
        if (~isempty(stimActionEyeMoveTrialsExt{extIdx,extIdx,eye}))
            resizedStimEyeExt{extIdx,eye} = cellfun(@(x) resample(x, minLengths(eye), length(x)), stimActionEyeMoveTrialsExt{extIdx,extIdx,eye}, 'UniformOutput', false);
            d = cell2mat(resizedStimEyeExt{extIdx,eye}(:)');
            muExt{extIdx,eye} = nanmean(d, 2);
            semExt{extIdx,eye} = nanstd(d, [], 2) ./ sqrt(size(d, 1));
        end
    end
    ...
    % Do it for extinction trials too
    for extIdx=1:2
        if (~isempty(muExt{extIdx,eye}))
            ySemExt{extIdx,eye} = [muExt{extIdx,eye}'-semExt{extIdx,eye}', fliplr(muExt{extIdx,eye}'+semExt{extIdx,eye}')];
        end
    end

    figure; hold on;
    skipLegend = zeros(1,totalNumStim+extraStim);
    for stimIdx=1:totalNumStim
        if (~isempty(mu{stimIdx,eye}))
    ...
    for extIdx=1:2        
        ...
        if (~isempty(ySemExt{extIdx, eye}))
            patch(ySemExt{extIdx,eye}, xSem, curShadingColor, 'EdgeColor', 'none');
            alpha(varianceAlpha);
            h = [h plot(muExt{extIdx,eye}, x, 'Color', curColor, 'LineWidth', lw)];
        end
    end    
    ...
    title([mouseName ' D' num2str(days) ': ' eyeName ' eye, CORRECT trials only'], 'Interpreter', 'none');
    annotation('textbox', [.8 0 .2 .2], 'String', ['n=' num2str(n)], 'FitBoxToText', 'on', 'EdgeColor', 'white');  
    if (length(numStim) == 1)
        if (numStim(1) == 3)
...
resizedStimEyeExt = cell(size(stimEyeMoveTrials));
muExt = cell(size(stimEyeMoveTrials));
semExt = cell(size(stimEyeMoveTrials));
ySemExt = cell(size(stimEyeMoveTrials));
for eye=1:2  % For each eye
    h = [];
    n = 0;
    ...
    for stimIdx=1:totalNumStim
        ...
        for actIdx=1:totalNumStim
            if (stimIdx ~= actIdx && ~isempty(stimActionEyeMoveTrials{stimIdx,actIdx,eye}))
                ...
                trialCnt(stimIdx) = trialCnt(stimIdx) + length(stimActionEyeMoveTrials{stimIdx,actIdx,eye});
            end
        end
    end
    ...
    % Next, add the extinction trials as well
    for extIdx=1:2
        trialCnt(totalNumStim + extIdx) = 0;
        for actIdx=1:totalNumStim
            if (extIdx ~= actIdx && ~isempty(stimActionEyeMoveTrialsExt{extIdx,actIdx,eye}))
                stimIncorrectEyeLengths = [stimIncorrectEyeLengths cellfun(@(x) length(x), stimActionEyeMoveTrialsExt{extIdx,actIdx,eye})];
                n = n + length(stimActionEyeMoveTrialsExt{extIdx,actIdx,eye});
                trialCnt(totalNumStim + extIdx) = trialCnt(totalNumStim + extIdx) + length(stimActionEyeMoveTrialsExt{extIdx,actIdx,eye});
            end
        end
    end
    ...
    for extIdx=1:2
        for actIdx=1:totalNumStim
           if (extIdx ~= actIdx && ~isempty(stimActionEyeMoveTrialsExt{extIdx,actIdx,eye}))
               cTmp = cellfun(@(x) resample(x, minLengths(eye), length(x)), stimActionEyeMoveTrialsExt{extIdx,actIdx,eye}, 'UniformOutput', false);
               resizedStimEyeExt{extIdx,eye} = cat(2,resizedStimEyeExt{extIdx,eye}, cTmp);
           end
        end
        if (~isempty(resizedStimEyeExt{extIdx,eye}))
            d = cell2mat(resizedStimEyeExt{extIdx,eye}(:)');
            muExt{extIdx,eye} = nanmean(d, 2);
            semExt{extIdx,eye} = nanstd(d, [], 2) ./ sqrt(size(d, 1));
        end
    ...
    title([mouseName ' D' num2str(days) ': ' eyeName ' eye, INCORRECT trials only'], 'Interpreter', 'none');
    annotation('textbox', [.8 0 .2 .2], 'String', ['n=' num2str(n)], 'FitBoxToText', 'on', 'EdgeColor', 'white');  
    if (length(numStim) == 1)
        if (numStim(1) == 3)
            standardLegend = {['left center (' num2str(trialCnt(1)) ')'], ['right center (' num2str(trialCnt(2)) ')'], ['center only (' num2str(trialCnt(3)) ')']};
...
% If only 1 day provided, plot the eye movements over time, color-coding the trials
% Plot stimulation and actions first
% Then plot inter-trial intervals
% Finally plot actual eye movement data
if (numDays == 1)  % Only plot elevation and azimuth for single days
