...
% The value will be a results vector of 3 entries, L, R or S actions.
% For the moment, we won't support opto
LC_map = containers.Map();
RC_map = containers.Map();
LO_map = containers.Map();
RO_map = containers.Map();
CO_map = containers.Map();
B_map = containers.Map(); % As of now, this will just have one entry, with a key of -1, since there are no targets and therefore no opacities
